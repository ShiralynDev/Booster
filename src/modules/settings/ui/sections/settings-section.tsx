...
export const SettingsSection = () => {
    ...
    const toggleAiContentMutation = trpc.users.toggleAiContent.useMutation({
        onMutate: async ({ enabled }) => {
            await utils.users.getByClerkId.cancel({ clerkId: clerkUserId });
            const previousUser = utils.users.getByClerkId.getData({ clerkId: clerkUserId });
        ...
        onSettled: () => {
            ...
            utils.search.getManyByQuery.invalidate();
            utils.search.getChannelsByQuery.invalidate();
            utils.videos.getMore.invalidate();
        },
        onSuccess: () => {
            toast.success("Settings updated");
        },
    ...
    const handleAiContentToggle = (checked: boolean) => {
        toggleAiContentMutation.mutate({ enabled: checked });
    };
            ...
            <div className="bg-card rounded-lg p-6 shadow-sm border space-y-6">
                <div>
                    ...
                    <div className="flex items-center justify-between p-4 border rounded-lg">
                        <div className="space-y-0.5">
                            <Label className="text-base">AI Content</Label>
                            <p className="text-sm text-muted-foreground">
                                Show videos labeled as AI generated
                            </p>
                        </div>
                        <Switch
                            checked={user?.aiContentEnabled ?? true}
                            onCheckedChange={handleAiContentToggle}
                        />
                    </div>
                </div>
            </div>
